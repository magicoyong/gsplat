...
> rasterize_forward_sum_gabor_tensor(
    ...
    if (channels == 3) {
        ...
        AT_ERROR("Gabor rasterization only supports 3 or 4 channels, got ", channels);
    }
...
rasterize_backward_sum_gabor_tensor(
    ...
    TORCH_CHECK(colors.ndimension() == 2 && (colors.size(1) == 3 || colors.size(1) == 4), 
                "colors must have dimensions (num_points, 3 or 4)");
    ...
    TORCH_CHECK(background.ndimension() == 1 && background.size(0) == channels,
                "background must match the color channels");
    ...
    // 关键修正：所有 float3/float4/float2/int2 类型必须先获取 float*/int* 指针，再进行 C 风格强转
    if (channels == 3) {
