...
__global__ void project_gaussians_2d_scale_rot_Norm_backward_kernel(
    ...
    v_mean2d[idx].x = v_xy[idx].x * (0.5f * img_size.x);
    v_mean2d[idx].y = v_xy[idx].y * (0.5f * img_size.y);
//     v_abs_mean2d[idx].x = v_abs_xy[idx].x * (0.5f * img_size.x);
//     v_abs_mean2d[idx].y = v_abs_xy[idx].y  * (0.5f * img_size.y);
//     v_abs_mean2d[idx].z = fabs(v_abs_xy[idx].z* (0.5f * img_size.x) );
//     v_abs_mean2d[idx].w =  fabs(v_abs_xy[idx].w * (0.5f * img_size.y));
}
...
__global__ void project_gaussians_2d_covariance_backward_kernel(
    ...
    // get v_cov2d
    cov2d_to_conic_vjp(conics[idx], v_conic[idx], v_cov2d[idx]); //对协方差的梯度
    // 再根据v_cov2d向前传播，计算v_L_elements
    float G_11 = v_cov2d[idx].x; // dL/dSigma_11
    float G_12 = v_cov2d[idx].y; // dL/dSigma_12, which is the same as dL/dSigma_21
    float G_22 = v_cov2d[idx].z; // dL/dSigma_22
    ...
    // COnv2d本身就是直接优化的参数 ltt
    // Calculate the gradients with respect to the elements of L
 
    // float3 cov2d = make_float3(l11, l21, l22);
    // float grad_l_11 =  2* G_11 ; // dL/dl_11
    // float grad_l_21 =  2*G_12 ; // dL/dl_21
    // float grad_l_22 = 2* G_22; // dL/dl_22
    float grad_l_11 =   G_11 ; // dL/dl_11
    float grad_l_21 =  G_12 ; // dL/dl_21
    float grad_l_22 =  G_22; // dL/dl_22
    ...
    v_mean2d[idx].y = v_xy[idx].y ;
//
//     v_abs_mean2d[idx].x = v_abs_xy[idx].x ;
//     v_abs_mean2d[idx].y = v_abs_xy[idx].y ;
//     v_abs_mean2d[idx].z = v_abs_xy[idx].z ;
//     v_abs_mean2d[idx].w = v_abs_xy[idx].w ;
...
//     v_abs_mean2d[idx].x = v_abs_xy[idx].x * (0.5f * img_size.x);
//     v_abs_mean2d[idx].y = v_abs_xy[idx].y * (0.5f * img_size.y);
//     v_abs_mean2d[idx].z = v_abs_xy[idx].z * (0.5f * img_size.x);
//     v_abs_mean2d[idx].w = v_abs_xy[idx].w * (0.5f * img_size.y);
//
//
// }
...
// ) {
//     unsigned idx = cg::this_grid().thread_rank(); // idx of thread within grid
//     if (idx >= num_points || radii[idx].x <= 0|| radii[idx].y <= 0) {
//         return;
//     }
//     // get v_cov2d
//     cov2d_to_conic_vjp(conics[idx], v_conic[idx], v_cov2d[idx]); //对协方差的梯度
//     // 再根据v_cov2d向前传播，计算v_L_elements
//     v_L_elements[idx].x =  v_cov2d[idx].x;
//     v_L_elements[idx].y = v_cov2d[idx].y;
//     v_L_elements[idx].z = v_cov2d[idx].z;
//
//     // v_mean2d[idx].x = v_xy[idx].x * (0.5f * img_size.x);
//     // v_mean2d[idx].y = v_xy[idx].y * (0.5f * img_size.y);
//     v_mean2d[idx].x = v_xy[idx].x ;
...
// ) {
//     unsigned idx = cg::this_grid().thread_rank(); // idx of thread within grid
//     if (idx >= num_points || radii[idx].x <radius_clip|| radii[idx].y <radius_clip) {
//         return;
//     }
//     // get v_cov2d
//     cov2d_to_conic_vjp(conics[idx], v_conic[idx], v_cov2d[idx]); //对协方差的梯度
