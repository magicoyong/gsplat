...
__global__ void nd_rasterize_backward_kernel(
    ...
    int2 range = tile_bins[tile_id];
        ...
        const int bin_final = min(final_index[pix_id], range.y - 1);
        for (int idx = bin_final; idx >= range.x; --idx) {
    ...
    float* __restrict__ v_means_t,
            ...
            for (unsigned c = 0; c < channels; ++c) {
                atomicAdd(&(v_rgb[channels * g + c]), alpha * v_out[c]);
            ...
            atomicAdd(
                &(v_xy[g].x),
                v_sigma * (conic.x * delta.x + conic.y * delta.y) + v_alpha * opac * gs_value * sin_sum_x
            );
            atomicAdd(
                &(v_xy[g].y),
                v_sigma * (conic.y * delta.x + conic.z * delta.y) + v_alpha * opac * gs_value * sin_sum_y
            );
            atomicAdd(&(v_opacity[g]), v_alpha * gs_value * H);
            ...
            for (int f = 0; f < num_freqs; ++f) {
                ...
                const float w = gabor_weights[g_idx];
                const float theta = delta.x * fx + delta.y * fy;
                atomicAdd(v_weights + g_idx, v_alpha * opac * gs_value * (-1.0f + __cosf(theta)));
                atomicAdd(v_freqs_x + g_idx, -v_alpha * opac * gs_value * w * delta.x * __sinf(theta));
                atomicAdd(v_freqs_y + g_idx, -v_alpha * opac * gs_value * w * delta.y * __sinf(theta));
            }
