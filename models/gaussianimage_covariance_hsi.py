...
class GaussianImage_Covariance_HSI(GaussianImage_Covariance):
    ...
    def __init__(self, loss_type="L2", **kwargs):
        ...
        # Match HSI_gabor / RGB Gabor_plus convention: initialize abundance logits so
        # softplus(_features_dc) ~= 0.05 with small jitter. Zero-init makes initial
        # softplus(0)=0.69 which oversaturates the abundance and slows convergence.
        ...
        # Parent already created self._cov2d = rand(N, 3) and self.cholesky_bound. The
        # effective cov2d = _cov2d + cholesky_bound is positive-definite by construction
        # whenever SLV bound is non-trivial, so we keep the parent's init.
    ...
    def initialize_new_covariance(self, num_points: int) -> torch.Tensor:
        # Match RGB Gabor_plus: produce raw cov2d that is itself positive-definite
        # so the densify-time check_non_semi_definite(new_cov2d) accepts most points.
        # The SLV cholesky_bound is added later in forward() / get_cov2d_elements.
